...
def quickstart(
    ...
    site_name: Annotated[str, typer.Option(help="Name for the site file.")] = "mysite",
    client: Annotated[str, typer.Option(help="LLM client: 'Claude' or 'Fireworks'.")] = "Fireworks",
) -> None:
    """Interactive quickstart: extract, compile, and generate first plan.

    This command guides you through the full pipeline:
    1. Extract rules from PDF using LLM
    2. Review extracted rules
    3. Compile to ASP
    4. Create skeleton site file
    5. Generate inspection plan

    Great for first-time users to see the full workflow.
    """
    from loguru import logger
    from rich.progress import Progress, SpinnerColumn, TextColumn
    from . import llm_extract

    console.print("\n[bold cyan]Compli Quickstart[/bold cyan]\n")
    console.print(f"PDF: {pdf.name}")
    console.print(f"Site: {site_name}\n")

    # Step 1: Extract rules
    console.print("[bold]Step 1/5:[/bold] Extracting rules from PDF...")

    rules_file = DATA / f"{pdf.stem}_rules.jsonl"
    ...
    try:
        with Progress(
            ...
            task = progress.add_task(f"Extracting rules using {client}...", total=None)
            n = llm_extract.extract_to_jsonl(pdf, rules_file, doc_name=pdf.stem, client=client)
            progress.update(task, description=f"✓ Extracted {n} rules", completed=True)

        console.print(f"[green]✓ {n} rules extracted[/green] -> {rules_file}\n")
    except Exception as e:
        console.print(f"[red]✗ Extraction failed: {e}[/red]")
        raise typer.Exit(1)

    # Step 2: Prompt for review
    console.print("[bold]Step 2/5:[/bold] Review rules")
    console.print(f"[dim]Run: compli review --rules-file {rules_file}[/dim]")

    if typer.confirm("\nReview rules now?", default=True):
        # Would call review command here, but for now just skip
        console.print("[yellow]Review step: run 'compli review --rules-file {}' manually[/yellow]\n".format(rules_file))
    else:
        console.print("[yellow]Skipping review (you can review later)[/yellow]\n")

    # Step 3: Auto-detect and compile chapters
    console.print("[bold]Step 3/5:[/bold] Compiling rules to ASP...")

    all_rules = read_jsonl(rules_file, Rule)
    chapters = sorted(set(r.clause_id.split('.')[0] for r in all_rules if '.' in r.clause_id))

    console.print(f"[cyan]Detected chapters:[/cyan] {', '.join(chapters)}")

    for ch in chapters:
        _compile_chapter(ch, rules_file, DATA / "clauses.jsonl", RULES_DIR, None)

    console.print(f"[green]✓ Compiled {len(chapters)} chapter(s)[/green]\n")

    # Step 4: Create skeleton site file
    console.print("[bold]Step 4/5:[/bold] Creating site file...")

    site_file = ROOT / "sites" / f"{site_name}.lp"
    site_file.parent.mkdir(parents=True, exist_ok=True)

    # Extract subject types from rules to scaffold site file
    subject_types = sorted(set(r.subject_type for r in all_rules))

    site_content = f"""% Site file for {site_name}
% Auto-generated by compli quickstart

% Site identity
site({site_name}).

% TODO: Add your equipment inventory below
% Based on detected subject types: {', '.join(subject_types)}

"""
    for i, subj_type in enumerate(subject_types[:5], 1):  # Limit to first 5
        site_content += f"% {subj_type}({subj_type}_1).  # Example - uncomment and customize\n"

    site_file.write_text(site_content)
    console.print(f"[green]✓ Created skeleton site file[/green] -> {site_file}")
    console.print(f"[yellow]Edit {site_file} to add your equipment[/yellow]\n")

    # Step 5: Show next steps
    console.print("[bold]Step 5/5:[/bold] Next steps")
    console.print(f"""
1. Edit site file: {site_file}
   Add your equipment (uncomment examples or add new ones)

2. Generate plan:
   compli plan --site {site_file} --chapter {','.join(chapters)}

3. Run inspection:
   compli inspect --site {site_file} --chapter {','.join(chapters)}
""")

    console.print("[green bold]✓ Quickstart complete![/green bold]")
    console.print("[dim]See docs/quickstart.md for full documentation[/dim]\n")
...
@app.command()
def compile(
    chapter: Annotated[str | None, typer.Option(help="Chapter letter, e.g. D. If omitted, auto-detects all chapters.")] = None,
    ...
    If --chapter is omitted, auto-detects all chapters from the rules file.
    """
    # Auto-detect chapters if not specified
    if chapter is None:
        all_rules = read_jsonl(rules_file, Rule)
        if not all_rules:
            console.print(f"[red]No rules found in {rules_file}[/red]")
            raise typer.Exit(1)

        # Extract unique chapter prefixes
        chapters = sorted(set(r.clause_id.split('.')[0] for r in all_rules if '.' in r.clause_id))

        if not chapters:
            console.print(f"[red]No chapters detected in {rules_file}[/red]")
            raise typer.Exit(1)

        console.print(f"[cyan]Auto-detected chapters:[/cyan] {', '.join(chapters)}")

        # Compile all chapters
        for ch in chapters:
            _compile_chapter(ch, rules_file, clauses_file, out_dir, site)
        return

    # Single chapter compilation
    _compile_chapter(chapter, rules_file, clauses_file, out_dir, site)
...
def _compile_chapter(
    ...
    source = compile_mod.compile_rules(rules, clauses, chapter, "compli", "0.1.0")
